...
- **Tests:** `tests/test_cockpit.py` (runner) + `tests/cockpit/` (13 suites, 192 tests) · `tests/test_hunt.py` (37 assertions) · `tests/test_backtest_daily.py` (12) · `tests/test_wrds_provider.py` · `tests/test_momentum_lib.py`. Run as plain scripts. **Only the first two gate** — the parked-track suites run in neither CI nor `deploy.sh`.
