...
- **§6.89** **F counts eight checks (user decision).** `scan._step2_summary(f, reaction)` adds Code 33 (`code33["all"]`), annual EPS up (`eps_fy_up`), estimates raised (`est_rev_90d ≥ 5`) and the last report held (a reaction exists and isn't a hard drop) to the four it had. `screen_universe` computes the reaction before the summary so `min_fundamental_score` sees all eight; `filter_candidates` still mirrors it by comparing `fund_score`. The app slider runs 0–8, the Step-2 caption reads "Score x/{checks}" (so an older scan's summary says /4) and the check row lists only the checks the summary has. The hunt adds `f_code33`/`f_fy`/`f_est`/`f_react` and `f_max`; the report reads F out of `f_max` (4 for a diagnostics.csv written before, which also lacks the new columns and renders with them as 0). The surprise stays out of F while its source is frozen (§12). Caches written before these keys existed are refetched at once (§6.90).
- **§6.90** **Old-schema caches blocked F for a week (found on the Pi, 2026-09-26).** The Pi still showed F out of 4 the morning after the eight-check commit was promoted. Two causes. (a) Friday's 16:20 screen ran on the previous image; the commit was promoted overnight, so the pickle held four checks (fund_score 0:45 1:80 2:96 3:153 4:80 over 454 rows). (b) That screen rewrote the ~450 passers' caches in the old schema, and `get_fundamentals` served any cache under 7 days old that had `next_earnings` and `last_surprise_pct`. None of the 1,582 caches had the new keys, so F would have read ≤ 4 for most passers until about 2026-10-02 (§6.89 had called this "up to a week" and treated it as acceptable). `_FUNDAMENTALS_KEYS` now includes `est_rev_90d`, and `_edgar_backfill` serves a fresh cache only if it has `_EDGAR_KEYS` (`code33`, `last_report`). The first screen after deploy refetches every passer; `cockpit-eod`'s log gives the cost.
- **§6.91** **Volume dry-up at the pivot (SEPA audit Step-3 item 1, "the most useful single filter it's missing").** `detect_vcp` measured each leg's volume against the 20 bars before its peak, never the final tight area or single quiet days. `advisories.volume_dryup` does (definition in §10). Row `dryup_ratio`/`dryup`, payload `dryup`; the Base column "Dry-up (× avg volume)", a Step-3 caption, the near-silent days marked on the chart's volume pane (`charts.build_chart(marks=)`, from `advisories.step3_marks`), and the hunt's `dryup_ratio`/`quiet_days`/`dryup` plus a "Base reads" column and `DU r/n` on each review sheet. **Pre-registered check, thresholds fixed before it ran** (`test_step3_reads_on_benchmark`): RLYB ("extreme vol dry-up") reads dry (0.08×, 4 quiet days), JAKK and BATRA dry, BEAM ("no dry-up") none. All four held. **The table says it does not separate the labels:** YES dry 13 · partial 15 · none 44; NO dry 23 · partial 40 · none 52 · n/a 13. Good charts read "none" more often (61% vs 41%). Part of that is the benchmark's construction: many YES charts were labelled after breaking out, and the final "contraction" the detector picks can be the loud pre-breakout push. Not tuned (§4); the read is advisory, and the guide says a dry-up alone doesn't make a setup.
- **§6.92** **Shakeouts (SEPA audit Step-3 item 3). The pre-registered check FAILED; the read ships as facts, without ✅/⚠️.** `advisories.shakeouts` (definition in §10): row `shakeout`/`undercut_broken`, payload `shakeouts`, a Base column "Shakeout", a neutral Step-3 caption, a mark under the undercut bar, hunt `shakeout` and `SO`/`LL` in "Base reads". **Check, fixed before it ran:** BOH and PNTG ("mid-base shakeout") show a shakeout, CLDX ("second low undercut the first") shows an undercut. PNTG (Jun 2, 2.3% under the Jun 1 low, back in 3 days) and CLDX held; **BOH did not**: its labelled "Jun 1 one-day shakeout" is the low of its own first contraction, not an undercut of an earlier low, so the definition can't see it. CLDX also shows the ambiguity: the human read its undercut as a flaw, the rule as a recovered shakeout (1.5%, back next day). The table has no signal either: YES shakeout 24 · broken 3 · clean 41 · n/a 4; NO shakeout 52 · broken 2 · clean 38 · n/a 36. Not tuned. A read that could see BOH would need reference lows the detector does not select (from before the first selected contraction, or its dropped sub-2-bar legs). Either is a change of its own, with its own pre-registration. **Taken off the app (user, 2026-09-26):** on the Pi's scan, 0.6–1.1% undercuts showed on most Tier-A names (MLAB, PUBM, ANET, TWIN, CNXN), so the caption, column and chart mark were removed. The read stays in the scan rows and the hunt's "Base reads".
- **§6.93** **V-shaped recovery (SEPA audit Step-3 item 4). The pre-registered check FAILED; the read ships as numbers only.** `advisories.v_recovery` (definition in §10). **Check, fixed before it ran:** the ⚠️ ships only if the flag fires on ≥ 7 of the 13 V-labelled NO charts and on ≤ 25% of YES. **Result: 0 of 13**, YES 1/72, other NO 8/115. Speeds on the V charts ran 0.3–2.7 (VTRS the only one over 2, at 14% depth). Likely cause: the base is measured from the first *selected* contraction, and the detector selects the tightening legs on the right side, after the V's low, so the fall the labeller saw sits before the base begins. Not tuned. The Step-3 caption gives depth, the two durations and the pace, for bases ≥ 15% deep only (on the Pi's scan it otherwise described a 6% HALO dip as a 3× "V"); the row carries `v_speed` (no table column: a flag column would be a verdict), the hunt `v_speed` and `R…x` in "Base reads". A V read that measures from the prior high, not the first selected peak, is a new design with its own pre-registration. **Taken off the app (user, 2026-09-26):** it describes the base's later stretch, not the V the books warn about, so the caption was removed; the hunt keeps `v_speed` and `R…x`.
- **§6.94** **The books' halving and the base length, shown not judged (SEPA audit Step-3 items 2 and 6, both "on purpose").** The detector's tightening rule and its 2-week minimum are deliberate (§10) and stay. `advisories.book_tightening` puts the books' rule of thumb beside them: each dip's depth over the one before, ✅ when every ratio is ≤ `BOOK_TIGHT_RATIO = 0.6` ("about half"). The caption also gives `base_length_weeks` against the books' 3 weeks, without a warning, since that measure under-reads. Row `book_tight`, payload `book_tightening`, hunt `book_tight` and `H` in "Base reads". Benchmark, printed only: YES halving 14 · not 54 · n/a 4; NO 20 · 72 · 36. The books' halving is rarer than the detector's rule on both labels, and no more common on the good charts.
...
- **The books' other setups — backburner (user, 2026-09-26; plan after Step 3).** The cockpit
  recognises only the VCP. Candidates, each a label on the chart and table, never a gate:
  - **Power play** (high tight flag): roughly doubles in ≤ 8 weeks on heavy volume, then rests
    3–6 weeks within ~20–25%. The one setup bought without strong earnings, so it would say
    "earnings not required" beside F. **Open decision:** power plays often fail the 8/8
    template (the 200-day hasn't been rising long); label them among passers only, or also
    keep a separate list of template failures.
  - **Flat base:** ≥ 5 weeks sideways within ~10–15% of the high, usually after an earlier
    breakout.
...
- **Tests:** `tests/test_cockpit.py` (runner) + `tests/cockpit/` (14 suites, 229 tests) · `tests/test_hunt.py` (48 assertions) · `tests/test_backtest_daily.py` (12) · `tests/test_wrds_provider.py` · `tests/test_momentum_lib.py`. Run as plain scripts. **Only the first two gate** — the parked-track suites run in neither CI nor `deploy.sh`.
