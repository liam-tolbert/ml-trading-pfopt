...
- The data refresh runs as a fresh one-shot container on two schedules, because
  ...
  - `cockpit-refresh.timer` — 09:30, then every :00/:30 through 15:30, plus
    16:10 ET weekdays. Tops up the watchlist **plus any held name not on it** (a
    position you cannot price blinds the Positions page and the sell pillars),
    then evaluates the triggers. Tens of names, seconds per run.
  - `cockpit-eod.timer` — 16:20 ET weekdays. **One unit, two sequential steps.**
    Step 1 tops up all ~4,100 scan-universe tickers after the settled close;
    because it writes post-settle it arms the settled-close cache serve, so every
    read until the next open costs zero network. Step 2 screens those bars into
    `last_scan.pkl`. `Type=oneshot` runs the two `ExecStart=` lines in order and
    skips step 2 if step 1 failed, so the screen can never read a half-updated
    cache and the two can never contend for yfinance.
- An hourly off-hours systemd timer (17:00–09:00 ET daily) runs `deploy.sh`:
...
- Write wear: `data/cockpit/prices/` is the hot path (~94 MB rewritten per full
  ...
  once-daily `cockpit-eod` cadence is comfortable — this was a real
